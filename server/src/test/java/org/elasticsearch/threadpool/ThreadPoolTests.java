...
import static org.elasticsearch.common.util.concurrent.EsExecutors.TaskTrackingConfig.DEFAULT_POOL_UTILIZATION_EWMA_ALPHA;
...
public class ThreadPoolTests extends ESTestCase {
    ...
    public void testDetailedUtilizationMetric() throws Exception {
        ...
        try {
            ...
            ExponentiallyWeightedMovingAverage expectedUtilizationEWMA = new ExponentiallyWeightedMovingAverage(
                DEFAULT_POOL_UTILIZATION_EWMA_ALPHA,
                0
            );
            for (int i = 0; i < 5; ++i) {
    ...
    private void runThreadUtilizationMetricCollection(
        ...
        // List<Measurement> measurements = meterRegistry.getRecorder().getMeasurements(
        // InstrumentType.DOUBLE_GAUGE,
        // ThreadPool.THREAD_POOL_METRIC_PREFIX + threadPoolName + ThreadPool.THREAD_POOL_METRIC_NAME_UTILIZATION
        // );
        // assertFalse(ThreadPool.THREAD_POOL_METRIC_NAME_UTILIZATION + " has no measurements", measurements.isEmpty());
        // assertThat(measurements.getLast().getDouble(), equalTo(0.0d));
        // expectedUtilizationEWMA.addValue(measurements.getLast().getDouble());
