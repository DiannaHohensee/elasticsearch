...
public final class TaskExecutionTimeTrackingEsThreadPoolExecutor extends EsThreadPoolExecutor {
    ...
    public double getPercentPoolUtilizationEWMA() {
        // NOMERGE: figure out the conditional dependencies on execution tracking.
        // Maybe add an assert to the TaskTrackingConfig object, if tracking utilization EWMA then execution tracking is on.
        if (trackOngoingTasks == false || trackUtilizationEWMA == false) {
            return 0;
        }
    ...
    protected void appendThreadPoolExecutorDetails(StringBuilder sb) {
        sb.append("task execution EWMA = ")
            ...
            .append(", ");
        if (trackQueueLatencyEWMA) {
            sb.append("task queue EWMA = ")
                .append(TimeValue.timeValueMillis((long) getQueuedTaskLatencyMillisEWMA()))
                .append(", ");
        }
        if (trackUtilizationEWMA) {
            sb.append("thread pool utilization percentage EWMA = ")
                .append(getPercentPoolUtilizationEWMA())
                .append(", ");;
        }
    }
