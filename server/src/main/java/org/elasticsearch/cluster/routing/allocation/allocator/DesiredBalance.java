 ...
 *
 * @param lastConvergedIndex TODO (Dianna)
...
public record DesiredBalance(
    ...
    enum ComputationFinishReason {
        ...
        CONVERGED,
        /** Computation was interrupted because a new cluster event occurred that affects computation */
        YIELD_TO_NEW_INPUT,
        /** Computation stopped early to avoid delaying new shard assignment */
        STOP_EARLY
    }
    ...
    private static int shardMovements(ShardAssignment old, ShardAssignment updated) {
        ...
        // started shards -- adding a shard copy is not a move -- by initializing the count with a negative value and then incrementing for
        // any new nodes.
